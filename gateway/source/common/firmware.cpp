...
int firmware_get_info(firmware_header_t* fh, const char* bin_file_path) {
	...
	for (index = 0; index < file_info.st_size; index += sizeof(uint32_t)) {
		temp_data = 0;
		int8_t status = pread(binary_file, &temp_data, sizeof(uint32_t), index);
		if(status == -1) {
			printf("cannot pread: 0x01");
		}
		check_sum += temp_data;
	}
...
int firmware_read(uint8_t* data, uint32_t cursor, uint32_t size, const char* bin_file_path) {
	...
	int8_t status = pread(binary_file, data, size, cursor);
	if(status == -1) {
		printf("cannot pread: 0x02");
	}
	close(binary_file);
